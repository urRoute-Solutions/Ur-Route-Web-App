...
const CATEGORIES = [
  { value: "OPERATOR_COMPLAINT", label: "Account / Profile Issue" },
...
export function RaiseTicketDialog({ urid }: { urid: string }) {
  const router = useRouter();
  const [open, setOpen] = useState(false);
  const [saving, setSaving] = useState(false);
  const [form, setForm] = useState({
    category: "OPERATOR_COMPLAINT",
    priority: "MEDIUM",
    subject: "",
    description: "",
  });
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    if (res.ok) {
      ...
      setOpen(false);
      setForm({ category: "OPERATOR_COMPLAINT", priority: "MEDIUM", subject: "", description: "" });
      router.refresh();
    } else {
