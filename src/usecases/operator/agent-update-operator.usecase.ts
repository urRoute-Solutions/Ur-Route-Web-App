...
import type { AgentUpdateOperatorInput } from "@/validators/operator";
import type { AuthPrincipal } from "@/types/auth";
import type { Operator } from "@prisma/client";

const EDITABLE_FIELDS = ["name", "description", "logoUrl", "contactEmail", "contactPhone", "address", "city"] as const;
...
export interface AgentUpdateOperatorResult {
  operator: OperatorDTO;
  ticketNumber: string;
  auditReference: string;
}

export async function agentUpdateOperatorUseCase(
  ticketId: string,
  input: AgentUpdateOperatorInput,
  principal: AuthPrincipal,
): Promise<AgentUpdateOperatorResult> {
  ...
  for (const field of EDITABLE_FIELDS) {
    ...
    const prev = (operator[field as keyof Operator] ?? "") as string;
    if (next !== prev) {
  ...
  // Update + resolve + audit are one atomic unit — compliance-critical state,
  // so it goes in a transaction rather than the fire-and-forget auditService helper.
  const { updated, auditRef } = await prisma.$transaction(async (tx) => {
    const updated = await tx.operator.update({ where: { id: operator.id }, data });
    ...
    const audit = await tx.auditLog.create({
      data: {
        action: "OPERATOR_UPDATED_BY_AGENT",
    ...
    await tx.serviceTicketMessage.create({
      data: {
        ...
        senderRole: "SYSTEM",
        body: `Operator details updated by ${agentName}: ${changeSummary}. Resolution: ${input.resolutionComments}. Ticket resolved.`,
      },
    });
  ...
  Promise.all([
    notificationService.sendEmail(
      ...
      operatorUpdatedEmailHtml(updated.name, changes, input.resolutionComments, ticketNumber(ticket.ticketSeq)),
    ),
    notificationService.sendInApp(
      operator.ownerId,
      "OPERATOR_UPDATED_BY_SUPPORT",
  ...
  ]).catch((err) => logger.error("Failed to send operator update notifications", { operatorId: operator.id, err }));

  return {
    operator: toOperatorDTO(updated),
    ticketNumber: ticketNumber(ticket.ticketSeq),
    auditReference: auditRef,
  };
}

function operatorUpdatedEmailHtml(
  companyName: string,
  ...
  return `
    <div style="font-family:system-ui,sans-serif;max-width:560px;margin:auto;padding:0">
      ...
      <div style="background:#fff;padding:32px;border-radius:0 0 12px 12px;border:1px solid #e5e7eb;border-top:none">
        <h2 style="color:#111;margin:0 0 8px;font-size:20px">Hi ${companyName},</h2>
